let
  isWayland = wayland.windowManager.sway.enable;
  fFox = if isWayland then pkgs.firefox-wayland else pkgs.firefox-bin;
  foxSettings = {
    "devtools.theme" = "dark";
    # Enable userContent.css and userChrome.css for our theme modules
    "toolkit.legacyUserProfileCustomizations.stylesheets" = true;
    # Stop creating ~/Downloads!
    "browser.download.dir" = "${home.homeDirectory}/Downs";
...
in
{
  systemd.user.sessionVariables = = mkIf isWayland "1";
  user.packages = [ fFox ];
  programs.firefox = {
    enable = true;
